...
namespace ProjectManagement.Controllers
{
    ...
    public class ProjectPhaseController : ControllerBase
    {
        ...
        public ProjectPhaseController(IGenericRepository<ProjectPhase> repo, IGenericRepository<Project> projectRepo, IMapper mapper)
        {
        ...
        public async Task<ActionResult> postProjectPhase(ProjectPhaseCreateDto model)
        {
            ...
            var project = await _projectRepo.GetById(model.ProjectId);
        ...
        public async Task<ActionResult> defaultProjectPhase(int id)
        {
            ...
            if(check.Count==0)
            {
                ProjectPhase obj = new ProjectPhase{
                    Name="To Do",Color="grey",ProjectId=project.Id
                };
                _repo.Insert(obj);
                ProjectPhase obj1 = new ProjectPhase{
                    Name="Pending",Color="yello",ProjectId=project.Id
                };
                _repo.Insert(obj1);
                ProjectPhase obj2 = new ProjectPhase{
                    Name="In Progress",Color="orange",ProjectId=project.Id
                };
                _repo.Insert(obj2);
                ProjectPhase obj3 = new ProjectPhase{
                    Name="In Review",Color="blue",ProjectId=project.Id
                };
                _repo.Insert(obj3);
                ProjectPhase obj4 = new ProjectPhase{
                    Name="Done",Color="green",ProjectId=project.Id
                };
                _repo.Insert(obj4);
                await _repo.Save();
                return Ok(new ApiErrorResponse(ErrorStatusCode.CreateSuccess));
